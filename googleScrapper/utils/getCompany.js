...
const oneSec = 1000;    // 1 second in milliseconds
...
async function getCompanyDataLinks(page) {
    try {
        ...
        await page.waitForSelector(resultsSelector, { visible: true, waitUntil: 'domcontentloaded', timeout: 5 * oneMin });
        ...
        const resultContainerHandle = await page.waitForSelector(allResultsSelector, { visible: true, waitUntil: 'domcontentloaded', timeout: 3 * oneMin });
...
async function getCompanyInfo(link, browser) {
    ...
    await companyPage.setDefaultNavigationTimeout(5 * oneMin); // Set a timeout of 5 minutes
